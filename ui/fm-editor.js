export const fmFields=[
  ...
  ['sl','Sustain level',15],['rr','Release rate',15],
];
export function readFmPatch(form) {
  ...
  return {algorithm:integer('algorithm',7),feedback:integer('feedback',7),
    operators:Array.from({length:4},(_,i)=>Object.fromEntries(fmFields.map(([key,,max])=>[key,integer(`op${i}_${key}`,max)])))};
}
export async function mountFmEditor(panel,invoke,onError) {
  ...
  <p>Six chip voices are shared across MIDI CH1–16. Apply changes the next Note On on this channel; held notes keep their current patch. Settings survive Stop and rack disable/enable, but reset when the app closes.</p>
  ...
  form.onsubmit=async event=>{
    ...
    try{const patch=readFmPatch(form);await invoke('synth_set_patch',{channel:ch,patch});
