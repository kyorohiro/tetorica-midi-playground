...
出力ポートとチャンネルをまとめた演奏先をJavaScriptから作成する。以下の固定IDによる複数音源・複数CH送信は試作実装済み。FILESの`examples/01_multi_output.js`で確認できる。
...
- [x] `await enableSoundChip("ym2612")` / `await enableSoundChip("sega-psg")`を追加。音源と接続先が利用可能になるまで待機し、初期化失敗は呼び出し元へ理由を返す。未知のIDはエラーにする。
- [x] 有効化済みの音源への再呼び出しは再初期化しない。並行呼び出しでも二重生成せず、既存の発音や設定をリセットしない。
...
実装メモ:
- 音源起動は現段階では共有Rack単位。`enableSoundChip`は両方の音源を起動し、既存Rackを再初期化しない。音源単位のEnable分離は残る。
- Nativeのノートキーを`(出力ID, CH, 音程)`に変更。所有者・寿命・外部Clockの期限を出力ごとに管理し、1出力の送信失敗でも他出力のNote Offを試みる。
- 同じOSポートは接続を共有。初回playで名前解決・接続し、固定IDは内蔵Rack有効確認＋一意なポート名で解決する。内蔵ポート所有IDの直接解決は今後強化する。
- 直接inline・引数なしliveLoopのハンドル呼び出しはループへ自動束縛。import先・別名ハンドル・引数ありcallbackは専用`playOutput`を明示的に使う。一般的なasync文脈の自動追跡は行わない。
- Stop時はスクリプト接続を解放し、Rackは保持。接続待ち後・Native送信時にRun世代を検証する。
- `examples/01_multi_output.js`をFILESへ追加済み。保存済み同名コードは上書きしない。ガイドと基本補完を追加。実機の同時発音は未確認。

### 名前指定とMIDIタブの論理出力（実装済み・実機確認は残る）
...
- [x] MIDIタブに`MIDI_OUTPUT_01`などの接続先割り当てを用意し、設定を保存する。スロットは01〜04、チャンネル省略時は1。
- [x] 固定ID・外部ポート名・論理出力を同じ演奏先interfaceへ解決する。論理出力は名前文字列と混同しない識別子にする。
- [x] 未設定・接続先消失・同名候補の重複を明確なエラーにする。列挙順による別ポートへの自動接続を避ける。
- [x] Run開始時に割り当てを固定し、`midi.output(MIDI_OUTPUT_XX)`作成時に未設定を検証し、未設定なら「MIDI_OUTPUT_02の接続先を設定してください」のように案内する。割り当て変更時はStopし、次のRunで反映する。接続先の存在・名前・IDは初回play時にNativeで再検証する。
- [x] 保存・復元・未設定・ポート消失と、同じ接続先を複数スロットで使う場合をテストする。

実装メモ: 外部ポートはID＋名前を保存し、不在・改名時に他の同名ポートへフォールバックしない。内蔵音源の固定IDも割り当て可能（既存の固定ID解決の制約は上記参照）。Run中のスナップショットはApplyでも保持。保存失敗時は以前の設定を保持し、エラーを表示する。`examples/02_assigned_outputs.js`は初期状態で内蔵音源を直接指定し、手動設定なしでRun可能。論理出力への切替はコメントで案内する。旧サンプルと完全一致する保存済みコードのみ更新し、ユーザーの編集は保持する。画面操作・再起動後の実機確認は残る。

実装順: 出力管理の調査 → 内蔵音源有効化と固定ID → 複数出力のノート所有権 → 論理出力設定 → 補完・examples。

### 共通の出力・ノート管理

- [x] 現在のWorker / UI / nativeの出力接続・ノート管理を調査し、複数出力への拡張方針を決める。
- [x] `midi.output(name, { channel })`の契約を定義する。チャンネルは1〜16、出力一覧の取得、名前の不一致・重複、接続タイミング、非同期エラーの伝え方を整理する。
- [x] 同じポートの接続を共有し、異なるポートへの同時送信に対応する。
- [x] 演奏先の`play()`の引数・戻り値・待機動作を決める。例のようにawaitせず発音予約でき、送信失敗も報告できるようにする。`duration`と`beat()`は既存MIDI版の拍単位に合わせる。`sleep()`はこの例のためだけに追加しない。
...
- [x] 模擬出力とWorker / nativeテストで、同一ポートのCH1・CH2、複数ポート、異常系と解放処理を検証する。
...
初心者が開いてRun fileに指定し、そのまま試せるコードを用意する。既存サンプルの名前を維持し、追加分を04以降とする。
...
- [x] `examples/04_first_note.js`: 内蔵YM2612を有効化し、固定IDで1音を鳴らす。
- [x] `examples/05_live_loop.js`: メロディの繰り返し、拍、音の長さ。
- [x] `examples/06_multi_channel.js`: 同じ出力のCH1 / CH2へ送る。
- [x] `examples/01_multi_output.js`: YM2612とSega PSGをコードで有効化し、両方へ同時に送る。
- [x] `examples/03_context_jsdoc.js`: 実装に即した`context`の使い方。
- [x] `examples/03_context_jsdoc.js`: 自作関数や設定のJSDoc補完。
- [x] 外部ポート名指定と`MIDI_OUTPUT_01`等の割り当てを使う追加例を用意する。内蔵音源のみの例とは準備手順を分ける。
- [x] 各例の冒頭に準備・期待する動作・変更して遊べる箇所を短く記載する。出力名の変更、受信側設定が必要な例では明記する。
- [x] FILESへの同梱・更新方法を整える。既存ユーザーの保存済みコードを上書きしない。
- [x] 英語を基本とし、日本語ガイドにもexamplesの入口と実行手順を記載する。
- [x] サンプルを模擬出力で実行し、送信先・チャンネル・ノートと終了時の解放を検証する。

実装メモ: 外部ポート名指定は`07_external_output.js`、論理出力設定は`08_output_slots.js`。必要な準備を冒頭と英日ガイドに明記。Worker＋模擬出力で送信先・CH・音程・ループ解放要求と未設定エラーを検証。Native側のNote Off処理は既存テストが担当し、実機での試聴は残る。
