...
- 外部MIDI Clockの監視とC4テスト。既定は内部BPM、外部拍待機は試験対応（play音長もパルス駆動）。
...
- [x] Run fileの直接inline `liveLoop(name, async () => { ... })`へ専用play/beat/nextBeat/cycleを構文解析で束縛。await後もループ単位で分離。明示引数とローカル宣言は維持。
- [ ] 別関数・import先・別名経由の暗黙コンテキストは未対応。これらは専用ヘルパーを引数で渡す。FM版との完全互換は継続課題。
...
- [x] ApplyでWorkerと拍・BPMを維持して再評価。同名ループは旧ノート解放後に置換しcycleはリセット、他のループは継続。Runは従来どおり全停止・再起動。削除したループはstopLoopで明示停止。評価中のApplyはログを出してスキップ。
...
- [x] 外部play音長をnativeのClockパルス期限へ変更。最小1/24拍、端数切り上げ、128拍まで。停止・途絶は既存のRun失効/Note Off。Workerの待機も外部拍を使用。
- [ ] 実機のテンポ変更中の音長、切断・再接続・IPC遅延を含む統合検証。
...
- 直近のJSテスト: 46件成功（外部Clockの受信世代・順序・24pulse転送を追加）。
