...
- [ ] 個別停止時の即時Note Off。同一チャンネル・同一音を使う複数ループの所有権を設計する。現状は送信済みノートの指定時間（最大10秒）でNote Off。画面Stopは全ノート解放。
...
- [ ] `noteLerp`：FM版の戻り値は音源固有ピッチ情報。MIDIでの丸め・Pitch Bend方針を決めてから実装。
